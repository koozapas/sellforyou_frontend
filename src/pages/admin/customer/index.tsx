...
const Customer = (props: Props) => {
	...
	} = useQuery<{ selectUsersByAdmin: User[] }, QuerySelectUsersByAdminArgs>(QUERIES.USER_LIST_BY_ADMIN, {
		fetchPolicy: 'no-cache',
	});

	const [extendAccount] = useMutation<{ extendMyAccountByUser: boolean }, MutationExtendMyAccountByUser>(
		MUTATIONS.EXTEND_MY_ACCOUNT_BY_USER,
		{},
	);

	const [userDetailQuery] = useLazyQuery<{ selectUsersByAdmin: User[] }, QuerySelectUsersByAdminArgs>(
	...
	const [userDetail2Query] = useLazyQuery<{ selectUsersByAdmin: User[] }, QuerySelectUsersByAdminArgs>(
		QUERIES.USER_LIST_BY_ADMIN,
		{
			onCompleted(data) {
				let result = data.selectUsersByAdmin;
	...
	const rowSelection = {
		...
		onSelect: (record: User, selected: boolean, selectedRows: User[], nativeEvent: any) => {
	...
	const columns = [
			...
			render: (data: User) => {
				return (
					<>
						...
						&nbsp;
						{data.userInfo.interparkCertKey !== '' && data.userInfo.interparkSecretKey !== '' ? (
							<img
						...
						&nbsp;
						{data.userInfo.lotteonApiKey !== '' && data.userInfo.lotteonVendorId !== '' ? (
							<img
		...
		{
			title: '상품 현황',
			render: (data: User) => <>{`${data.productCount}/${data.userInfo?.maxProductLimit ?? '무제한'}`}</>,
			align: 'center',
			width: '10%',
		},
		{
		...
		},
		{
			...
			render: (data: User) => {
				...
				if (plans.length > 0) {
					return <>{format(new Date(data.purchaseInfo.levelExpiredAt), 'dd')}</>;
				}
	...
	return (
		<>
			...
			>
				...
				>
					<Item label='아이디'>{currentData?.email}</Item>
					...
					<Item label='휴대폰 번호'>{currentData?.userInfo?.phone}</Item>

					<Item label='이용중인 서비스'>
						<span style={{ whiteSpace: 'pre-line' }}>
							{currentData?.purchaseInfo2
								? transformPurchaseInfo(currentData?.purchaseInfo2, {
										isNeedDate: true,
										itemDelimiter: '\n',
									})
								: ''}
					...
					<Item label='결제(승인) 내역'>
						{currentData?.purchaseInfo?.history
							? JSON.parse(currentData?.purchaseInfo?.history).map((v: any, i: number) => {
									...
									return (
										<div>
											결제 완료: {detail.name} 승인 ({format(new Date(v.purchasedAt), 'yyyy.MM.dd HH:mm')} ~{' '}
					...
					<Item label='상품 현황'>
						<>{currentData ? `${currentData.productCount}/${currentData.userInfo?.maxProductLimit ?? '무제한'}` : ''}</>
					</Item>
				</Descriptions>
				...
				<Descriptions size='small' bordered column={2}>
					<Item label='스마트스토어 주소'>{currentData?.userInfo?.naverStoreUrl}</Item>

					<Item label='위메프 ID'>{currentData?.userInfo?.wemakepriceId}</Item>

					<Item label='쿠팡 아이디'>{currentData?.userInfo?.coupangLoginId}</Item>

					<Item label='쿠팡 업체코드'>{currentData?.userInfo?.coupangVendorId}</Item>

					<Item label='쿠팡 액세스키'>{currentData?.userInfo?.coupangAccessKey}</Item>

					<Item label='쿠팡 시크릿키'>{currentData?.userInfo?.coupangSecretKey}</Item>
					...
					<Item label='인터파크 상품등록 인증키'>{currentData?.userInfo?.interparkCertKey}</Item>

					<Item label='인터파크 상품등록 시크릿키'>{currentData?.userInfo?.interparkSecretKey}</Item>

					<Item label='인터파크 상품수정 인증키'>{currentData?.userInfo?.interparkEditCertKey}</Item>

					<Item label='인터파크 상품수정 시크릿키'>{currentData?.userInfo?.interparkEditSecretKey}</Item>

					<Item label='G마켓 ID'>{currentData?.userInfo?.esmplusGmarketId}</Item>

					<Item label='옥션 ID'>{currentData?.userInfo?.esmplusAuctionId}</Item>

					<Item label='롯데온 Vendor ID'>{currentData?.userInfo?.lotteonVendorId}</Item>

					<Item label='롯데온 API 키'>{currentData?.userInfo?.lotteonApiKey}</Item>

					<Item label='티몬 ID'>{currentData?.userInfo?.tmonId}</Item>
				</Descriptions>
			</Modal>
		</>
	);
