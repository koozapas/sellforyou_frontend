	...
	useEffect(() => {
		...
		if (window.location.pathname.includes('admin')) {
			...
			// }
				...
				('/admin/sales/sales-by-term' || '/admin/sales/sales-by-product' || '/admin/sales/sales-by-store')
			) {
				setOpenKeys(['매출통계']);
			} else if (
				window.location.pathname ===
				('/admin/customer-center/notice' || '/admin/customer-center/faq' || '/admin/customer-center/inquiry')
			) {
				setOpenKeys(['고객센터']);
		...
		>
			...
			>
				{sideMenuElement.map((value, index) =>
					value.list.length === 0 ? value?.grade && subMenuNoChild(value) : value?.grade && subMenuHasChild(value),
				)}
